...
def partial_sums(s):
    "*** YOUR CODE HERE ***"
    def compute_rest():
        return stream_map(lambda x: s.first + x, partial_sums(s.rest))
...
def pairs(s, t):
    "*** YOUR CODE HERE ***"
    def compute_rest():
        top_row = pairs(s,t.rest)
        bottom_third = pairs(s.rest, t.rest)
        return interleave(top_row, bottom_third)
    return Stream((s.first, t.first), compute_rest)
