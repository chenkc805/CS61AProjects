...
def make_tweet_fn(text, time, lat, lon):
    ...
    "*** YOUR CODE HERE ***"
    return lambda picker: {'text':text, 'time':time, 'lat':lat, 'lon':lon}[picker]
...
def extract_words(text):
    ...
    "*** YOUR CODE HERE ***"
    for letters in text:
        if letters not in ascii_letters:
...
def analyze_tweet_sentiment(tweet):
    ...
    """
    "*** YOUR CODE HERE ***"
    words_with_sentiment = [sentiment_value(get_word_sentiment(words)) for words in tweet_words(tweet) 
                            if has_sentiment(get_word_sentiment(words)) != False]
    if words_with_sentiment == []:
        return make_sentiment(None)
    else:
        sum_sentiment_values = sum(words_with_sentiment)
        mean_sentiment = sum_sentiment_values / len(words_with_sentiment)
        return make_sentiment(mean_sentiment)
...
def find_centroid(polygon):
    ...
    "*** YOUR CODE HERE ***"
    def lat_point(i):
        return latitude(polygon[i])
    def lon_point(i):
        return longitude(polygon[i])
    area = 0.5 * sum(lat_point(i)*lon_point(i+1) - lat_point(i+1)* lon_point(i) for i in range(len(polygon)-1))
    if area == 0:
        return [lat_point(0),lon_point(0),0.0]
    else:
        centroid_x = (sum([(lat_point(i)+lat_point(i+1)) * (lat_point(i)*lon_point(i+1)- lat_point(i+1)*lon_point(i)) for i in range(len(polygon)-1)])) / (6 * area)
        centroid_y = (sum([(lon_point(i)+lon_point(i+1)) * (lat_point(i)*lon_point(i+1)- lat_point(i+1)*lon_point(i)) for i in range(len(polygon)-1)])) / (6 * area)
        return [centroid_x,centroid_y,abs(area)]

def area_of_polygon(polygon):
    return find_centroid(polygon)[-1]
...
###################################
def distances_to_states(tweet):
    return {geo_distance(tweet_location(tweet),find_state_center(us_states[key])):key for key in us_states}

def closest_state(tweet):
    return distances_to_states(tweet)[min(distances_to_states(tweet))]
...
def group_tweets_by_state(tweets):
    ...
    """
    "*** YOUR CODE HERE ***"
    return group_by_key([[closest_state(a_tweet), a_tweet ] for a_tweet in tweets])

def remove_none_sentiments(tweets_by_state):
    for key in tweets_by_state:
        for tweets in tweets_by_state[key]:
            if has_sentiment(analyze_tweet_sentiment(tweets)) == False:
                tweets_by_state[key].remove(tweets)
                remove_none_sentiments(tweets_by_state)
    return tweets_by_state

def remove_empty_key(tweets_by_state):
    for key in tweets_by_state:
        if tweets_by_state[key] == []:
...
def average_sentiments(tweets_by_state):
    ...
    """
    "*** YOUR CODE HERE ***"
    tweets_by_state = remove_none_sentiments(tweets_by_state)
    tweets_by_state = remove_empty_key(tweets_by_state)
    return {key:sum([sentiment_value(analyze_tweet_sentiment(tweet)) 
            for tweet in tweets_by_state[key] if has_sentiment(analyze_tweet_sentiment(tweet)) != False])
            /len([tweet for tweet in tweets_by_state[key]])  
            for key in tweets_by_state}
