...
def brack_read(tokens):
    ...
    100
    >>> print(brack_read(tokenize('(10 5)')))
    (- 10 5)
    >>> brack_read(tokenize('([])'))
    Pair('-', Pair(Pair('+', nil), nil))
    >>> print(brack_read(tokenize('<[2{12 6}](3 4 5)>')))
    (* (+ 2 (/ 12 6)) (- 3 4 5))
    >>> print(brack_read(tokenize('[2{12 6}]')))
    (+ 2 (/ 12 6))
    ...
    if n != None:
        ...
        "*** YOUR CODE HERE ***"
        tails_read = read_tail(tokens, token)
        return Pair(brackets[token,left_to_right[token]], tails_read)

def read_tail(tokens, token):
    if not tokens:
        raise SyntaxError('unexpected end of line')
    n = coerce_to_number(tokens[0])
    if tokens[0] == left_to_right[token]:
        return nil
    elif tokens[0] in left_to_right:
        a = brack_read(tokens)
        tokens.pop(0)
        return Pair(a,read_tail(tokens,token))
    elif n != None:
        tokens.pop(0)
        return Pair(n, read_tail(tokens, token))
    else:
        raise SyntaxError("unexpected {0}".format(tokens[0]))
...
class Mobile:
    ...
    def __init__(self, left, right):
        "*** YOUR CODE HERE ***"
        if isinstance(left, Branch) and isinstance(right, Branch):
            self.left = left
...
def interpret_mobile(s):
    ...
    for t in ('Branch', 'Weight', 'Mobile'):
        "*** YOUR CODE HERE ***"
        if interpret_mobile(s.replace('T', t, 1)) != None:
            return interpret_mobile(s.replace('T', t, 1))
    return None
...
def unique(s):
    ...
    "*** YOUR CODE HERE ***"
    def helper(s, t):
        if s.rest == Stream.empty:
            return to_stream(t)
        elif s.first not in t:
            t.append(s.first)
        return helper(s.rest, t)
    lst = [] 
    return helper(s,lst)
...
def keep_if(fn, s, max_run_length):
    def compute_rest():
        return keep_if(fn, s.rest, max_run_length-1)
    if max_run_length == 0:
        return s
    if s == Stream.empty:
        return Stream.empty
...
def count(name, s, max_run_length):
    if max_run_length == 0:
        return 0
    if s == Stream.empty:
        return 0
        ...
        if s.first == name:
            return count(name, s.rest, max_run_length-1) + 1
        else:
            return count(name, s.rest, max_run_length-1)
...
def rle(s, max_run_length=10):
    ...
    "*** YOUR CODE HERE ***"
    def compute_rest():
        return rle(reduced_stream, max_run_length)
    reduced_stream = keep_if(lambda x: x != s.first, s, max_run_length)
    return Stream((count(s.first, s, max_run_length), s.first), compute_rest)
