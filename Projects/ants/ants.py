...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant == None:
                self.ant = insect
            elif self.ant.container is True:
                assert insect.container is False, 'Both ants are bodyguards'
                assert self.ant.ant is None, 'Bodyguard already contains an ant'
                self.ant.contain_ant(insect)
            elif insect.container is True:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            if insect.name == 'Bodyguard' and insect.ant != None:
                self.ant = insect.ant
                return
            assert self.ant == insect, '{0} is not in {1}'.format(insect, self)
            # Phase 4: Special handling for QueenAnt
            "*** YOUR CODE HERE ***"
            if insect in doubled_ants:
                doubled_ants.remove(insect)
            if insect.name == 'Queen' and insect.id == 0:
                print('Cannot remove Queen')
                return
            self.ant = None
        else:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if self.blocked():
            ...
            if self.place.exit.ant == None:
                self.move_to(self.place.exit)
                return
            if self.place.ant == None:
                if self.place.exit.ant.name == 'Queen' or (self.place.exit.ant.name == 'Bodyguard' and self.place.exit.ant.ant.name == 'Queen'):
                    colony.queen.bees.append(self)
                    return
                else:
                    self.move_to(self.place.exit)
                    return
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        return (self.container is True) and (self.ant == None) and (other.container is False)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        "*** YOUR998 CODE HERE ***"
        if last_element(self.place) not in zero_to_nine and self.place != 'Hive':
            self.place = self.place.entrance.exit
        self_place_int = int(last_element(self.place))
        check_place = self.place
        while check_place != 'Hive':
            if last_element(check_place) not in zero_to_nine:
                return None
            check_place_int = int(last_element(check_place))
            difference = abs(check_place_int - self_place_int)
            if difference >= self.min_range and difference <= self.max_range:
                if random_or_none(check_place.bees) != None:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            for every_bee in self.place.bees[:]:
...
class ScubaThrower(ThrowerAnt):
    """ A subclass of ThrowerAnt that is more costly and watersafe, but otherwise identical to its base class."""
    name = 'Scuba'
    food_cost = 5
    implemented = True
    watersafe = True
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        if a_bee is not None:
            a_bee.reduce_armor(a_bee.armor)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.id >= 1:
            ...
            if self.place.ant.name != 'Queen' and self.place.ant not in doubled_ants:
                self.place.ant.damage *= 2
                doubled_ants.append(self.place.ant)
            while place_backward.exit != None:
                if (place_backward.exit.ant != None) and (place_backward.exit.ant not in doubled_ants):
                    place_backward.exit.ant.damage *= 2
                    doubled_ants.append(place_backward.exit.ant)
                if (place_backward.exit.ant != None) and place_backward.exit.ant.container == True and place_backward.exit.ant.ant != None and place_backward.exit.ant.ant not in doubled_ants:
                    place_backward.exit.ant.ant.damage *= 2
                    doubled_ants.append(place_backward.exit.ant.ant)
                place_backward = place_backward.exit
            while place_forward.entrance != None:
                if (place_forward.entrance.ant != None) and (place_forward.entrance.ant not in doubled_ants):
                    place_forward.entrance.ant.damage *= 2
                    doubled_ants.append(place_forward.entrance.ant)
                if (place_forward.entrance.ant != None) and place_forward.entrance.ant.container == True and place_forward.entrance.ant.ant != None and place_forward.entrance.ant.ant not in doubled_ants:
                    place_forward.entrance.ant.ant.damage *= 2
                    doubled_ants.append(place_forward.entrance.ant.ant)
