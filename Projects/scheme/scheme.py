...
def scheme_optimized_apply(procedure, args, env):
    """Apply Scheme PROCEDURE to argument values ARGS in environment ENV."""
    if isinstance(procedure, PrimitiveProcedure):
        return apply_primitive(procedure, args, env)
    elif isinstance(procedure, LambdaProcedure):
        "*** YOUR CODE HERE ***"
        new_frame = procedure.env.make_call_frame(procedure.formals, args)
        return procedure.body, new_frame
    elif isinstance(procedure, MuProcedure):
        "*** YOUR CODE HERE ***"
        mu_frame = env.make_call_frame(procedure.formals, args)
        return procedure.body, mu_frame
    else:
        raise SchemeError("Cannot call {0}".format(str(procedure)))
...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("A TypeError has been made")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            ...
            raise SchemeError("cannot assign to literal")
    else:
...
def do_let_form(vals, env):
    ...
    for i in range(len(bindings)):
        if not scheme_symbolp(bindings[i].first):
            raise SchemeError("binding not a variable")
        if len(bindings[i].second) != 1:
            raise SchemeError("wrong number of values per binding")
...
def check_formals(formals):
    ...
    while formals:
        if type(formals.first) in [tuple, list, float, int, Pair, bool] or formals.first == nil:
            raise SchemeError("Parameters must be valid")
        elif formals.first not in lst_of_formals:
            lst_of_formals.append(formals.first)
            formals = formals.second
        else:
            raise SchemeError("Two or more identical variables used")
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            if isinstance(procedure, PrimitiveProcedure):
                ...
                expr, env = scheme_optimized_apply(procedure, args, env)
