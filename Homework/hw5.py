...
def has_sublist(s, sublist):
    ...
    """
    "*** YOUR CODE HERE ***"
    if s == sublist or has_prefix(s, sublist):
        return True
    elif s == empty:
        return False
    else:
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount, attempt):
        if attempt in good_passwords:
            return withdraw(amount,old_password)
        else:
            return withdraw(amount,attempt)

    good_passwords, one_withdraw = [], withdraw(0,old_password)

    if type(one_withdraw) == str:   #incorrect password
        return one_withdraw
    else:                           #correct password
        if type(one_withdraw) == int:
            good_passwords.append(old_password)
            good_passwords.append(new_password)
        return joint
