...
(define (zip pairs)
   (cond ((eq? pairs '()) '(() ()))
      (else (cons (cons (caar pairs) (car (zip (cdr pairs))))  
                  (cons (cons (cadar pairs) (cadr (zip (cdr pairs)))) nil)))
    ))
...
(define (partitions total max-value)
  (cond
    ...
    ((= max-value 0) nil)
    (else 
      (define yes (partitions (- total max-value) max-value))
      (define no  (partitions total (- max-value 1)))
      (define add-max-value (lambda (s) (cons max-value s)))
      (define yes (apply-to-all add-max-value yes))
      (extend yes no))
    ))

(define (list-partitions total max-pieces max-value)
  (keep-if (lambda (x) (>= max-pieces (length x))) (partitions total max-value)
  ))
