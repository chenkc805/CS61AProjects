...
('string)
; expect Error: cannot call string
...
(/ (- (+ 1 9 8 9) 2 0 1 4) 2 5)
; expect Error: A TypeError has been made
...
(define volume (* 1.333 pi (cube-r radius)))
volume
; expect Error: unknown identifier: volume
...
; expect cone-volume
(cone-volume 3 6)
; expect 56.492071380000006

(define (cylinder-surface-area r h)
  (+ (* 2 pi r h) (* 2 pi (square r))))
; expect cylinder-surface-area
(cylinder-surface-area 2 5)
; expect 87.96452
...
'(1 2 'inception '!!!)
; expect (1 2 (quote inception) (quote !!!))

(begin (define a 1) (/ 1 0))
...
(define difference (lambda (x y) (- x y)))
; difference

(difference 5 2)
; expect 3

((lambda (x y z) (+ x y (square z)) 1 2))
...
(define (inverse-function y) (/ 1 y))
; expect inverse-function

(inverse-function 2)
; expect 0.5

(inverse function 2)
; expect Error: unknown identifier inverse

(define (123) abc)
; expect Error: cannot assign to literal

(define ('stringwheee) abc)
; expect Error: cannotassign to literal
...
(odd? 'string)
; expect Error: operand 0 (string) is not a number
...
(and or)
; expect Error: unknown identifier or
...
(or and or)
; expect Error: unknown identifer and
...
(define a 'first)
(define z 'last)
(let ((a 1) (z 26)) (list a z))
; expect (1 26)

(list a z)
; expect (first last)

(let john-denero 1337)
; expect Error: bad bindings list in let form

(let cs-is-cool 1)
; expect Error: bad bindings list in let form

(let where-is-my-expr)
