...
def flatten_rooted_tree(t):
    if is_rooted(t) == False:
        return [t]
    else:
        return sum([flatten_rooted_tree(b) for b in t],[])
# Q2
def size_of_tree(t):
...
def flatten_linked_list(t):
    if is_link(t) == False:
        return [t]
    elif  t == 'empty':
        return []
    else:
        return sum([flatten_linked_list(b) for b in t],[])
...
def sum_linked_list(lst, fn):
    ...
    """
    "*** YOUR CODE HERE ***"
    flat = flatten_linked_list(lst)
    return sum([fn(i) for i in flat])
